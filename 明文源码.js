    ...
    try {
      ...
      if (!upgradeHeader || upgradeHeader !== 'websocket') {
        ...
        if (路径 == '/') {
          ...
          const fakeConfig = await 生成配置信息(userID, request.headers.get('Host'), sub, 'CF-Workers-SUB', RproxyIP, url, fakeUserID, fakeHostName, env, 100 * 1024 * 1024); // 设置总流量限制为100M
...
<tg-spoiler>入口: ${url.pathname + url.search}</tg-spoiler>`);
          const 维列斯Config = await 生成配置信息(userID, request.headers.get('Host'), sub, UA, RproxyIP, url, fakeUserID, fakeHostName, env, 100 * 1024 * 1024); // 设置总流量限制为100M
          const now = Date.now();
          today.setHours(0, 0, 0, 0);
          const UD = Math.floor(((now - today.getTime()) / 86400000) * 24 * 1099511627776 / 2);
          let pagesSum = UD;
          let workersSum = UD;
          let total = 100 * 1024 * 1024; // 总流量限制为 100M (100 * 1024 * 1024 字节)
